...
def add(a: int, b: float, c: float = 3.0, **kwargs) -> float:
    return (a + b) / c


cfg = Config(
    _validate=lambda cfg: int(cfg.model.n_layers) > 0,
    model=Config(
        _groups={
            "_default": "rnn",
            "rnn": Config(
                n_layers=Field(type=int, default=32, help="Number of RNN Layers"),
            ),
            "transformer": Config(
                n_layers=Field(type=int, default=64, help="Number of Transformer Layers"),
                num_attention_heads=Field(
                    type=int,
                    default=8,
                    help="Number of Attention Heads",
                ),
                a=Field(reference_root=lambda cfg: cfg.simple.a),
                submodel=Config(a=5, b=7, c=Field(reference=lambda cfg: cfg.a + cfg.b)),
            ),
        }
    ),
    add=Config(_from_signature=add),
    batch_size=16,
    simple=Config(a="abc", _validate=lambda simple: len(simple.a) > 2),
    name=Field(),
)

# repr(cfg)
# overrides: list[str] = "--model transformer --model.n_layers 128 --add.b 3.0 --add.a 2 --add.c 4 --name test".split(" ")  # type: ignore
# # cfg.override({"model": {"_group": "rnn", "n_layers": 256}})
